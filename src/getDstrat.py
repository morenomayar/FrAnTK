...
USAGE="""
Compute D statistics of the form D(h1, h2; h3, h4), using (allele frequency) definition and normalization in Patterson et al, 2012 (Genetics). We only consider sites where there are (minder, maxder] derived alleles in h1+h2+h3+h4 together. 
	...
	for line in f:
		...
		if ancf == 1:
			...
			for i in (h1p, h2p, h3p, h4p):
				nofder+=int(l[2+5+i*3])
				nofanc+=int(l[1+5+i*3])
			...
			nofcop=[]
			for i in (h1p, h2p, h3p, h4p):
				nofder+=int(l[1+5+i*3])
				nofanc+=int(l[2+5+i*3])
...
try:
	...
	Z=Dstat/jackSE
	Z
	O=open(resdir+"/Dstrat_"+"_".join([freqprefbase, h1, h2, h3, h4, anc, str(minder), str(maxder), str(mincops)])+".txt", 'w')
