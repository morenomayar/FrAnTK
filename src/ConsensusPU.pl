...
while(<STDIN>){
	...
	#check if depth >= 3
	if($Alength>=3){
		#sort allele count array
		@sort_ac=sort{$b<=>$a} @AC;
		#check if there is a majority consensus allele
		if($sort_ac[0] > $sort_ac[1]){
			#get sum of three minor alleles
			$sum3=$sort_ac[1]+$sort_ac[2]+$sort_ac[3];
			#check if larger count is actually the majority
			if($sort_ac[0] > $sum3){
				#if it is, then we get the majority base
				for($i=0; $i<4; $i++){
					if($AC[$i]==$sort_ac[0]){
						$cns=$bases[$i];
						last;
