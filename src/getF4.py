...
USAGE="""
Compute D statistics of the form D(h1, h2; h3, h4), using (allele frequency) definition and normalization in Patterson et al, 2012 (Genetics).
...
with subprocess.Popen(["zcat",freqpref+"_freqs.gz"],stdout=subprocess.PIPE).stdout as f:
	for line in f:
		...
		try:
			...
			continue
		#this comparison should not be here because it is possible to have top=0
		#if top == 0:
			#continue
...
try:
	...
	Z=Dstat/jackSE
	Z
	O=open(resdir+"/f4_"+"_".join([freqprefbase, h1, h2, h3, h4])+".txt", 'w')
