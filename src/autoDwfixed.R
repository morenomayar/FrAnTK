...
PossibleArgNames<-c("freqpref", "h1", "h2", "h3", "h4", "catfile", "legfile", "nthr", "bsize", "resfile", "f4", "rmtrns", "plotdnd")
...
USAGE<-("
	...
	plotdnd\t(0 | 1)\tPlot dendrograms on top. Default: 1
...
f4<-as.integer(ArgHash[["f4"]])
rmtrns<-as.integer(ArgHash[["rmtrns"]])
plotdnd<-as.integer(ArgHash[["plotdnd"]])
...
if(!(plotdnd ==1 | plotdnd ==0)){
	message(USAGE)
	stop("plotdnd can only be 0 or 1")
}
...
if(is.na(resfile)){
	...
	if(is.na(h1)){
		a<-pops
		a<-a[pops!=h2 & pops!=h3 & pops!=h4]
		cats<-cats[pops!=h2 & pops!=h3 & pops!=h4]
	...
	if(is.na(h2)){
		a<-pops
		a<-a[pops!=h1 & pops!=h3 & pops!=h4]
		cats<-cats[pops!=h1 & pops!=h3 & pops!=h4]
	...
	if(is.na(h3)){
		a<-pops
		a<-a[pops!=h1 & pops!=h2 & pops!=h4]
		cats<-cats[pops!=h1 & pops!=h2 & pops!=h4]
...
if(!is.na(resfile)){
	...
	if(identical(Fixed, c(F,T,T,T))){
		...
		Fixed3<-Results[1,4]
		if(f4==0){
			MainText<-paste(sep="", "D(H1, ", Fixed1, "; ", Fixed2, ", ",  Fixed3, ")\n(", Fixed1, ",", Fixed2, ") <---> (H1,", Fixed2, ")")
		}else{
			MainText<-paste(sep="", "f4(H1, ", Fixed1, "; ", Fixed2, ", ",  Fixed3, ")\n(", Fixed1, ",", Fixed2, ") <---> (H1,", Fixed2, ")")
		}
		...
		Fixed3<-Results[1,4]
		if(f4==0){
			MainText<-paste(sep="", "D(", Fixed1, ", H2; ", Fixed2, ", ",  Fixed3, ")\n(", "H2", ",", Fixed2, ") <---> (", Fixed1, ",", Fixed2, ")")
		}else{
			MainText<-paste(sep="", "f4(", Fixed1, ", H2; ", Fixed2, ", ",  Fixed3, ")\n(", "H2", ",", Fixed2, ") <---> (", Fixed1, ",", Fixed2, ")")
		}
		...
		Fixed3<-Results[1,4]
		if(f4==0){
			MainText<-paste(sep="", "D(", Fixed1, ", ", Fixed2, "; H3, ", Fixed3, ")\n(", Fixed2, ",", "H3", ") <---> (", Fixed1, ",", "H3", ")")
		}else{
			MainText<-paste(sep="", "f4(", Fixed1, ", ", Fixed2, "; H3, ", Fixed3, ")\n(", Fixed2, ",", "H3", ") <---> (", Fixed1, ",", "H3", ")")
		}
	...
	heightval<-ifelse(dim(Results)[1]>=52, dim(Results)[1]*8/52, 8)
	h125<-heightval
	widthval<-ifelse(length(table(Results$Cat))==1, 5, 6.5)
	lorows<-ceiling(heightval*4/1.8)
	if(lorows>125){
		h125<-(125*heightval)/lorows
		lorows<-125
	}

	if(plotdnd==1){
	pdf(paste(sep="", resfile, ".pdf"), height=heightval, width=widthval, useDingbats=F)
	#Do layout for plot with trees
	lo<-matrix(0,lorows,80)
	#this is if there is no legend
	if(length(table(Results$Cat))==1){
		lo[1,5:78]<-1
		lo[1:4,13:35]<-2
		lo[1:4,36:58]<-3
		lo[1:4,59:80]<-4
	#this is when there is a legend
	}else{
		#Used to be +3, i.e., 13:29, 33:51, 54:70
		#on 311020, removed 1, used to be 10:26, 30:48, 51:67
		lo[1,5:78]<-1
		lo[1:4,9:25]<-2
		lo[1:4,29:47]<-3
		lo[1:4,50:66]<-4
	...
	layout(lo)
	par(mar=c(0,0,0,0))
	#first position of layout (empty)
	plot(1, col="white", xaxt="n", yaxt="n", bty="n")
	par(mar=c(3.6, 2, 2, 1))

	#trees
	m<-matrix(c(c(1,1,3,3), c(1,1,1,2), c(1,1,1,1), c(1,4,4,4)), 4,4,byrow=TRUE)
	...
	m<-matrix(c(c(1,1,1,1), c(1,1,3,3), c(1,1,1,2), c(1,4,4,4)), 4,4,byrow=TRUE)
	#m<-matrix(c(c(1,1,1,1), c(1,1,1,2), c(1,1,3,3), c(1,4,4,4)), 4,4,byrow=TRUE)
	hc<-hclust(dist(m))
	hc$labels<-TreeLabs
	hc$order<-c(2,1,3,4)
	...
	#put everything together with grid
	if(max(nchar(hc$labels))<=9){
		#this part was the original version (without ifelse for h125)
		#else part is extra
		our.first.vp <- viewport(x = 0.5, y = 0, height=ifelse(lorows>125, (h125-1.5)/heightval, (heightval-1.5)/heightval), width = 1, just = c("centre", "bottom"))
	}else{
		extrachars<-max(nchar(hc$labels))-9
		extraspace<-extrachars*.0625
		our.first.vp <- viewport(x = 0.5, y = 0, height=ifelse(lorows>125, (h125-1.5-extraspace)/heightval, (heightval-1.5-extraspace)/heightval), width = 1, just = c("centre", "bottom"))
	}
