...
PossibleArgNames<-c("freqpref", "h1", "h2", "h3", "h4", "x", "nthr", "bsize", "resfile", "rmtrns", "minp", "maxp", "pstep", "catfile", "legfile")

USAGE<-("
	Compute admixture subtracted f4 statistics of the form (f4(h1, h2; h3, h4)-padm*f4(h1, x; h3, h4))/(1-padm), using (allele frequency) definition in Reich et al, 2012 (Nature). We compute f4_subtr for values of padm within the range [minp, maxp], with step size pstep.
	...
	x\tSTR\tName of population to set in x. Required
	minp\tFloat. Minumum value of padm. Default=0.05.
	maxp\tFloat. Maximum value of padm. Default=0.5.
	pstep\tFloat. Step size for range [minp, maxp]. Default=0.05.
...
#default values for bsize, nthreads and padms
...
if(sum(!is.na(ArgHash[["h1"]]), !is.na(ArgHash[["h2"]]), !is.na(ArgHash[["h3"]]), !is.na(ArgHash[["h4"]]), !is.na(ArgHash[["x"]]))!=5 & is.na(ArgHash[["resfile"]])){
	message(USAGE)
	stop("h1, h2, h3, h4 and x are required.")
}
...
if(!is.na(ArgHash[["minp"]]) & !is.na(ArgHash[["maxp"]]) & !is.na(ArgHash[["pstep"]])){
	minp<-as.numeric(ArgHash[["minp"]])
	maxp<-as.numeric(ArgHash[["maxp"]])
	pstep<-as.numeric(ArgHash[["pstep"]])
	#check if pvals could be cast as float
	if(is.na(minp) | is.na(maxp) | is.na(pstep)){
		message(USAGE)
		stop("minp, maxp and pstep should be numeric. ")
	}
	#if they are numeric, check they are nonnegative
	if(minp<0 | maxp<0 | pstep <0){
		message(USAGE)
		stop("minp, maxp and pstep should be nonnegative. ")
	}
	#check minp and maxp bounds
	if(minp<0 | maxp>=1){
		message(USAGE)
		stop("minp should be >=0 and maxp should be <1. ")
	}

	#check minp<maxp
	if(minp>=maxp){
		message(USAGE)
		stop("minp should be <maxp. ")
	}
	#check pstep is within the range
	pdiff<-maxp-minp
	if(pstep>pdiff){
		message(USAGE)
		stop("pstep should be <(maxp-minp). ")
	}
	#create range
	padmrange<-seq(minp, maxp, pstep)
	if(!(maxp %in% padmrange)){
		padmrange<-c(padmrange, maxp)
	}
	if(!(0 %in% padmrange)){
		padmrange<-c(0, padmrange)
	}
	...
	message(USAGE)
	stop("minp, maxp and pstep are required.")
}

#check if we have required args or resfile
if((is.na(freqpref) | is.na(h1) | is.na(h2) | is.na(h3) | is.na(h4) | is.na(x) | is.na(minp) | is.na(maxp) | is.na(pstep)) & is.na(ArgHash[["resfile"]])){
	message(USAGE)
	stop("freqpref, h1, h2, h3, h4, x, minp, maxp and pstep are required")
}
...
if(is.na(resfile)){
	...
	#build cmds
	cats<-cats[pops==h2]
	resdir<-paste(sep="_", Dpref, h1, h2, h3, h4, x, as.character(minp), as.character(maxp), as.character(pstep), freqprefbase, RandExt)
	cmds<-paste(Dprog, " h1=", h1, " h2=", h2, " h3=", h3, " h4=", h4, " x=", x, " padm=", as.character(padmrange), " freqpref=", freqpref, " resdir=", resdir, " bsize=", as.character(bsize), " category=", cats, sep="")
	#filenames will have the names of the output files that are produced
	filenames<-paste(sep="", resdir, "/", Dpref, freqprefbase, "_", h1, "_", h2, "_", h3, "_", h4, "_", x, "_", as.character(padmrange), ".txt")
	#print(filenames)
	...
	names(restab)<-c("H1pop", "H2pop", "H3pop", "H4pop", "xpop", "f4", "SE", "Z", "nSNPs", "nBlocks", "Plot", "Cat")
	#get adm props
	padms<-sapply(strsplit(unlist(strsplit(filenames, ".txt$")), "_"), function(x){x[length(x)]})
	restab<-cbind.data.frame(restab, padms, stringsAsFactors=F)
...
if(!is.na(resfile)){
	...
	if(length(unique(Results$H1pop))!=1 | length(unique(Results$H2pop))!=1 | length(unique(Results$H3pop))!=1 | length(unique(Results$H4pop))!=1 | length(unique(Results$xpop))!=1){
		message(USAGE)
		stop("All columns H1pop, H2pop, H3pop, H4pop, xpop should be fixed. ")
	}

	Results<-Results[Results$Plot,]
	Results$f4<-as.numeric(Results$f4)
	...
	Results$padms<-as.numeric(Results$padms)
	Results<-Results[rowSums(is.na(Results))==0,]
	Upper<-Results$f4+Results$SE
	Lower<-Results$f4-Results$SE
	#This is a 99.9ci
	Upper3<-Results$f4+qnorm(1-(.001)/2)*Results$SE
	Lower3<-Results$f4-qnorm(1-(.001)/2)*Results$SE
	Results<-cbind(Results, Upper, Lower, Upper3, Lower3)

	MainText<-paste(sep="", "(f4(", Results$H1pop[1], ",", Results$H2pop[1], ";", Results$H3pop[1], ",", Results$H4pop[1], ")\n-c*f4(", Results$H1pop[1], ",", Results$xpop[1], ";", Results$H3pop[1], ",", Results$H4pop[1], "))/(1-c)\n(", Results$H2pop[1], ",", Results$H3pop[1], ")<--->(", Results$H1pop[1], ",", Results$H3pop[1], ")")

	MainTextY<-paste(sep="", "Subtracted ", Results$xpop[1], " fraction (c) from\nf4(", Results$H1pop[1], ",", Results$H2pop[1], ";", Results$H3pop[1], ",", Results$H4pop[1], ")")
	...
	F4PLOT<-ggplot(Results, aes(x=f4, y=padms, color=Cat, fill=Cat, shape=Cat))+
	...
	geom_text(data=Results[-Results$f4<=0, ], aes(x=Lower3, y=padms, label=z), hjust=1.05, show.legend=F, size=2.7)+
	geom_text(data=Results[-Results$f4>0, ], aes(x=Upper3, y=padms, label=z), hjust=-.05, show.legend=F, size=2.7)

	pdf(paste(sep="", resfile, ".pdf"), height=4, width=4, useDingbats=F)
