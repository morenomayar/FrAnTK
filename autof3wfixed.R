...
USAGE<-("
	...
	NOTE: Exactly two of h1,h2,target are required. 
	...
	legfile\tSTR\tTab separated file specifying outer color, inner color and pch for each category (used for plotting and legend). Random colors and pch are chosen for categories not present in the file. Format: category outercolor innercolor pch. (Use R color names and pch numbers) .
...
if(sum(!is.na(ArgHash[["h1"]]), !is.na(ArgHash[["h2"]]), !is.na(ArgHash[["target"]]))!=2 & is.na(ArgHash[["resfile"]])){
	message(USAGE)
	stop("exactly 2 pops should be specified from h1, h2, target. ")
}
...
if(is.na(resfile)){
	...
	if(is.na(h1)){
		a<-pops
		a<-a[pops!=h2 & pops!=target]
		cats<-cats[pops!=h2 & pops!=target]
		resdir<-paste(sep="_", Dpref, "h1", h2, target, freqprefbase, RandExt)
		cmds<-paste(Dprog, " h1=", a, " h2=", h2, " target=", target, " freqpref=", freqpref, " resdir=", resdir, " bsize=", as.character(bsize), " category=", cats, sep="")
		#filenames will have the names of the output files that are produced
		filenames<-paste(sep="", resdir, "/f3_", freqprefbase, "_", a, "_", h2, "_", target, ".txt")
	}
	...
	#if target is fixed, h1 and h2 are variable
	if(is.na(target)){
		a<-pops
		a<-a[a!=h1 & a!=h2]
		resdir<-paste(sep="_", Dpref, "target", h1, h2, freqprefbase, RandExt)
		cmds<-paste(Dprog, " target=", a, " h1=", h1, " h2=", h2, " freqpref=", freqpref, " resdir=", resdir, " bsize=", as.character(bsize), " category=", cats, sep="")
		#filenames will have the names of the output files that are produced
		filenames<-paste(sep="", resdir, "/f3_", freqprefbase, "_", h1, "_", h2, "_", a, ".txt")
	}
